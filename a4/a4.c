...
// these are regular dynamic variables on stack
   ...
   for (proc=0; proc<MAXPROCESSES; proc++) { 
      ...
      if (q[proc].active) {
           ...
           //count active pages; 
           //if count == 1 && pc ==0 then load 4 into memeory.
           int count = 1;
           int i ; // for loop;
           for(i =1; i<MAXPROCPAGES; i++){
              if(q[proc].pages[i] >0) {count++; break;}
           }

           //first time a process is loaded; => we load 5 pages at a time;
           if(pc == 0 && count < 2){
                countPAGE = page + 5;
            }
           ...
           for( ; page<countPAGE; page++){
                   if (!pagein(proc,page)) {   // try to swap in, if this fails: 
                    ...
                    for(i = 0; i< MAXPROCESSES; i++) { 
                      if( (q[proc].pages[i] >0) && (timestamps[proc][i] <= minimum) ) {  
